...
When running MMT with Launchpad Authentication, MMT should be configured to run at `https://mmt.localtest.earthdata.nasa.gov`. The url will need to be configured to redirect to `localhost:3000`:
  ...
  1. **Modify `/etc/hosts`
    Append 'mmt.localtest.earthdata.nasa.gov' to the 127.0.0.1 localhost line**
     ...
     2. Append 'mmt.localtest.earthdata.nasa.gov' to 127.0.0.1
        1. **127.0.0.1 localhost mmt.localtest.earthdata.nasa.gov**
  ...
  2. **Install Nginx**
     ...
     1. **Generate the self-signed certificate (see references below)**
        1. Create a directory
           ```
           mkdir cretificate
           ```
        2. Inside the certificate directory, create a mmt.localtest.earthdata.nasa.gov.conf file
           ```
           sudo nano mmt.localtest.earthdata.nasa.gov.conf
           ```
        3. Paste the following into the configuration file:
            ```
           [ req ]
            default_bits        = 2048
        ...
        5. Create the self signed Certificate
            ```
           `openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout mmt.localtest.earthdata.nasa.gov.key -out mmt.localtest.earthdata.nasa.gov.crt -config mmt.localtest.earthdata.nasa.gov.conf`
           ...
           *** When prompted with questions for the certificate, leave them blank so information from the configuration file will be used by default ***
           ```
        6. Copy the Certificate Key Pair to the nginx folder
            ```
        ...
        7. Add the certificate to Mac OSX Keychain
            1. Open the certificate in Keychain Access
                ```
               open /usr/local/etc/nginx/mmt.localtest.earthdata.nasa.gov.crt
               ```
            2. Ensure the certificate is added to the System keychain
            3. Ensure the certificate is trusted:
                ```
               1. Double click on the certificate
               2. Open the Trust section
               3. For the dropdown next to 'When using this certificate' choose 'Always Trust'
               4. You may need to enter your password to make these changes
                ```

        8. Update the Nginx Configuration File to load the certificate key pair
           1. `nano /usr/local/etc/nginx/nginx.conf`
           2. Paste the following at the end of nginx.conf file:
              ```
  ...
  4. Run the rails server on the appropriate port:
      ```
     rails s
     ```
...
You should be able to visit MMT at https://mmt.localtest.earthdata.nasa.gov which will redirect you to Launchpad for authentication and back to the https url you added
