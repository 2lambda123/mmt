    ...
    if (keys.length > 0) {
      ...
      rows.push(
        <div className="error-list-item" onClick={() => { this.navigateToField(errorProperty) }} key={key} data-testid={`error-list-item__${kebabCase(key)}`}>
          {key !== 'root' ? (
            <li>
              {editor.hasVisitedSection(section.displayName) ? <i key={key} data-testid={`error-list-item__${kebabCase(key)}--error`} className="eui-icon eui-icon--sm eui-fa-times-circle red-progress-circle" />
                : <i data-testid={`error-list-item__${kebabCase(key)}--info`} className="eui-icon eui-icon--sm eui-fa-circle-o gray-progress-circle" />}
