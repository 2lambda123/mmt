...
export default class MetadataEditor {
  ...
  hasVisitedSection(displayName: string) {
    const sections = (this.model.visitedSections.map((item) => item))
    sections.pop()
    return sections.includes(displayName)
  }

  // Intents
  navigateTo(section: FormSection) {
    this.model.currentSection = section
    this.model.addToVisitedSections(section.displayName)
  }
